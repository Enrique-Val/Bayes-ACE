...
def check_graph(bayesian_network, dataset : pd.DataFrame, graph_type, distance_threshold, log_likelihood_threshold, acc_thresh) :
    ...
    ## CHECK THE GRAPH
    path_file = os.path.dirname(__file__) + "/face_vars/graph_weights_" + str(graph_type) +"_" + str(distance_threshold) + "_" + str(log_likelihood_threshold) + "_" + str(acc_thresh) + ".pkl"
    ...
    path_file = os.path.dirname(__file__) + "/face_vars/cfxpath_" + str(graph_type) + "_" + str(
        distance_threshold) + "_" + str(log_likelihood_threshold) + "_" + str(acc_thresh) + ".pkl"
    #with open(path_file, "wb") as file :
...
if __name__ == "__main__":
    ...
    for i in product(list_gt,list_eps, list_ll,list_acc) :
        check_graph(bn, df, i[0], distance_threshold = i[1], log_likelihood_threshold= i[2], acc_thresh = i[3])
