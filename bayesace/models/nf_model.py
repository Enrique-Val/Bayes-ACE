...
data = get_and_process_data(44123).drop("class", axis=1)
...
spline_transform = T.spline_coupling(n_dims)
spline_transform_2 = T.spline_coupling(n_dims)
flow_dist = dist.TransformedDistribution(base_dist, [spline_transform,spline_transform_2])
...
validation_loss = -torch.stack(
        [
            flow_dist.log_prob(batch_data[0])
            for batch_data in X_val_dataloader
        ],
        -1,
    ).mean()
