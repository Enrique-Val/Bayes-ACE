...
from bayesace.models.NF_base.nf_estimator import NfEstimator
...
def create_single_nf(args, i, data_loaders, seed=0,
                       verbose=False) -> NfEstimator:
...
class MultiNf:
    def __init__(self, args, data, seed=0, parallelize=False):
        ...
        if parallelize:
            ...
            result = pool.starmap(create_single_nf, [(args, i, class_data_loaders[i], seed, args.verbose) for i in
                                                       class_data_loaders.keys()])
            pool.close()
            for i, label in enumerate(class_data_loaders.keys()):
                self.nfs[label] = result[i]
        else:
            for label in class_data_loaders.keys():
                self.nfs[label] = create_single_nf(args, label, class_data_loaders[label], seed, args.verbose)
    ...
    def logl(self, data, class_var_name="class"):
        ...
        for i in self.class_dist.keys():
            ...
            if len(index_i) == 0:
                continue
            logl_i = self.nfs[i].compute_log_p_x(data_i.values).detach().cpu().numpy()
    ...
    def likelihood(self, data, class_var_name="class"):
        ...
        to_ret = np.zeros(data.shape[0])
        for i in self.class_dist.keys():
            logl_i = self.nfs[i].compute_log_p_x(data).detach().cpu().numpy()
    ...
    def predict(self, data: np.ndarray):
        ...
        for i, label in enumerate(self.class_dist.keys()):
            logl_i = self.nfs[label].compute_log_p_x(data).detach().cpu().numpy().astype(float) + np.log(self.class_dist[label])
            ll = ll + np.e ** logl_i
            acc[i] = np.e ** logl_i
