...
from bayesace.algorithms.algorithm import ACEResult, ACE
...
if __name__ == "__main__":
    ...
    construction_time_df = pd.DataFrame(columns=["construction_time"], index=["face_baseline", "face_kde", "face_eps", "bayesace_nf", "bayesace_clg"])

    t0 = time.time()
    face_baseline = FACE(bayesian_network=gt_estimator, features=df_train.columns[:-1], chunks=chunks,
                         dataset=df_train.drop("class", axis = 1),
                         distance_threshold=5, graph_type="integral", f_tilde=None, seed=0, verbose=False,
                         likelihood_threshold=0.00, accuracy_threshold=0.00, penalty=1)
    tf = time.time()-t0
    construction_time_df.loc["face_baseline", "construction_time"] = tf

    t0 = time.time()
    face_kde = FACE(bayesian_network=normalizing_flow, features=df_train.columns[:-1], chunks=chunks,
                        dataset=df_train.drop("class", axis = 1),
                        distance_threshold=5, graph_type="kde", f_tilde=None, seed=0, verbose=False,
                        likelihood_threshold=0.00, accuracy_threshold=0.00, penalty=1)
    tf = time.time()-t0
    construction_time_df.loc["face_kde", "construction_time"] = tf

    t0 = time.time()
    face_eps = FACE(bayesian_network=normalizing_flow, features=df_train.columns[:-1], chunks=chunks,
                    dataset=df_train.drop("class", axis = 1),
                    distance_threshold=5, graph_type="epsilon", f_tilde="identity", seed=0, verbose=False,
                    likelihood_threshold=0.00, accuracy_threshold=0.00, penalty=1)
    tf = time.time()-t0
    construction_time_df.loc["face_eps", "construction_time"] = tf

    t0 = time.time()
    bayesace_nf = BayesACE(bayesian_network=normalizing_flow, features=df_train.columns[:-1],
                           n_vertex=n_vertex,
    ...
    tf = time.time()-t0
    construction_time_df.loc["bayesace_nf", "construction_time"] = tf

    t0 = time.time()
    bayesace_clg = BayesACE(bayesian_network=clg_network, features=df_train.columns[:-1],
                            n_vertex=n_vertex,
                            accuracy_threshold=0.00, likelihood_threshold=0.00,
                            chunks=chunks, penalty=penalty, sampling_range=sampling_range,
                            initialization="default",
                            seed=0, verbose=False, pop_size=100)
    tf = time.time()-t0
    construction_time_df.loc["bayesace_clg", "construction_time"] = tf
    ...
    algorithms:list[ACE] = [face_eps, bayesace_nf, bayesace_clg] #
    model_str_list = ["face_baseline", "face_kde", "face_eps", "bayesace_nf", "bayesace_clg"][2:]
